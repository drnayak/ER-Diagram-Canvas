  ...
  return (
    <div className="w-72 shrink-0 flex flex-col bg-white border-l border-gray-200 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <div>
          <p className="text-xs text-gray-400 uppercase tracking-widest font-semibold mb-0.5">Table Details</p>
      ...
      <div className="flex-1 overflow-y-auto">
        {(table.module || table.description) && (
          <div className="px-4 py-3 border-b border-gray-50">
            {table.module && (
              <span className={`text-[11px] px-2 py-0.5 rounded-full font-semibold capitalize ${MODULE_BADGE[table.module] ?? "bg-gray-100 text-gray-600"}`}>
                {table.module}
              </span>
            )}
            ...
            <div className="space-y-1.5">
              {table.indexes!.map((idx) => (
                <div key={idx.name} className="bg-gray-50 rounded-lg px-2 py-1.5">
                  <p className="text-[11px] font-semibold text-gray-700 font-mono">{idx.name}</p>
                  <p className="text-[10px] text-gray-400 mt-0.5">[{idx.columns.join(", ")}]</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {(relatedFrom.length > 0 || relatedTo.length > 0) && (
          <div className="px-4 py-3">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Relationships</p>
            </div>
            {[...relatedFrom, ...relatedTo].map((rel, i) => (
              <div key={i} className="text-xs py-1.5 border-b border-gray-50 last:border-0">
                <span className="text-[10px] bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded font-semibold">{rel.type}</span>
