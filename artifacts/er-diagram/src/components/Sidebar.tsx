...
const MODULE_META: Record<string, { color: string }> = {
  ...
  sales:     { color: "bg-blue-500" },
  inventory: { color: "bg-emerald-500" },
  warehouse: { color: "bg-amber-500" },
  finance:   { color: "bg-rose-500" },
  hr:        { color: "bg-pink-500" },
  logistics: { color: "bg-cyan-500" },
  crm:       { color: "bg-indigo-500" },
  admin:     { color: "bg-gray-500" },
  reporting: { color: "bg-teal-500" },
};
...
export default function Sidebar() {
  ...
  const grouped = schema.tables.reduce(
    (acc, table) => {
      const mod = table.module ?? "other";
      if (!acc[mod]) acc[mod] = [];
      acc[mod].push(table);
      return acc;
    },
    ...
    <div className="flex flex-col h-full bg-gray-950 text-gray-100 w-64 shrink-0 border-r border-gray-800">
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {Object.entries(filteredGrouped).map(([mod, tables]) => {
          const allGroupSelected = tables.every((t) => selectedTables.has(t.table_name));
          const someGroupSelected = tables.some((t) => selectedTables.has(t.table_name)) && !allGroupSelected;
          ...
          return (
            <div key={mod} className="mb-1">
              <div className="flex items-center px-3 py-2 hover:bg-gray-800/50">
                ...
                />
                <div className={`w-2 h-2 rounded-full ${getModuleColor(mod)} mr-2 shrink-0`} />
                <span className="text-xs font-semibold text-gray-300 flex-1 capitalize">{mod}</span>
                <span className="text-[10px] text-gray-600">{tables.length}</span>
              </div>
                ...
                <div className="ml-7">
                  {tables.map((table) => (
                    <label
                      ...
                      className="flex items-center gap-2 px-3 py-1.5 cursor-pointer hover:bg-gray-800/40 rounded"
                    >
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className={`text-xs truncate ${selectedTables.has(table.table_name) ? "text-gray-200" : "text-gray-500"}`}>
                        {table.table_name}
                      </span>
                    </label>
                  ))}
